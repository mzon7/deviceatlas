#!/usr/bin/env python3
"""
Process missed PMA supplements using the OpenFDA bulk PMA JSON file.
No API calls — reads the bulk file directly, no rate limits.
"""
...
BULK_FILE = "/tmp/pma-bulk/device-pma-0001-of-0001.json"
...
def main():
    ...
    """)
    # Index by submission_number
    db_by_pma = {r["submission_number"]: r for r in db_rows}
    print(f"Found {len(db_rows)} DB records for missed PMAs")
    ...
    # Stream bulk file and collect records for our missed PMAs
    print(f"\nStreaming bulk file...")
    # Group by pma_number, keeping only supplements (non-empty supplement_number)
    pma_records = {}  # pma_number -> list of supplement records sorted by decision_date desc

    with open(BULK_FILE, "r", encoding="utf-8", errors="replace") as f:
        data = json.load(f)

    all_results = data.get("results", [])
    print(f"Total records in bulk file: {len(all_results):,}")

    for rec in all_results:
        pma_num = (rec.get("pma_number") or "").strip()
        supplement_num = (rec.get("supplement_number") or "").strip()
        if pma_num not in MISSED_SET:
            continue
        if not supplement_num:
            continue  # Skip base PMA records, only keep supplements
        if pma_num not in pma_records:
            pma_records[pma_num] = []
        pma_records[pma_num].append(rec)

    print(f"Found supplement records for {len(pma_records)} of {len(MISSED_PMAS)} missed PMAs")

    # Sort each PMA's supplements by decision_date descending
    for pma_num in pma_records:
        pma_records[pma_num].sort(
            key=lambda r: r.get("decision_date") or "0000",
            reverse=True
        )

    # Process each missed PMA
    updates = []
    new_devices = []
    processed = 0

    for pma_num in MISSED_PMAS:
        device = db_by_pma.get(pma_num)
        if not device:
            continue  # Not in DB at all

        results = pma_records.get(pma_num, [])
        if not results:
            continue
        ...
        processed += 1
        print(f"  {pma_num}: {len(results)} supplements, newest={most_recent_date}, +{added} new devices")
