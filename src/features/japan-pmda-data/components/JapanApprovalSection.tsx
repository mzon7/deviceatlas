import type { Approval } from "../../device-profile-overview/lib/useDeviceProfile";
import StatusBadge from "../../device-profile-overview/components/StatusBadge";

interface JapanApprovalSectionProps {
  approvals: Approval[];
}

const COLOR = "#BC002D";        // Japan red
const LIGHT_BG = "rgba(188,0,45,0.03)";
const BORDER = "rgba(188,0,45,0.12)";
const HEADER_BG = "rgba(188,0,45,0.05)";

function pmdaSearchUrl(deviceName: string) {
  return `https://www.pmda.go.jp/english/review-services/reviews/approved-information/devices/0001.html`;
}

export default function JapanApprovalSection({ approvals }: JapanApprovalSectionProps) {
  const hasIndications = approvals.some((a) => a.disease_state != null);
  const latestApproval = approvals[0];

  return (
    <div
      style={{
        background: "rgba(255,255,255,0.75)",
        backdropFilter: "blur(12px)",
        WebkitBackdropFilter: "blur(12px)",
        borderRadius: 16,
        border: `1px solid ${BORDER}`,
        boxShadow: "0 2px 16px rgba(0,0,0,0.03)",
        overflow: "hidden",
      }}
    >
      {/* Header */}
      <div
        style={{
          display: "flex",
          alignItems: "center",
          gap: 10,
          padding: "18px 24px",
          borderBottom: `1px solid ${BORDER}`,
          background: HEADER_BG,
          flexWrap: "wrap",
        }}
      >
        <span style={{ fontSize: 22 }}>🇯🇵</span>
        <span style={{ fontWeight: 700, fontSize: 16, color: "#111" }}>
          Japan (PMDA)
        </span>
        {approvals.length > 0 && (
          <span
            style={{
              background: COLOR,
              color: "#fff",
              borderRadius: 12,
              padding: "2px 10px",
              fontSize: 12,
              fontWeight: 700,
            }}
          >
            PMDA Approved
          </span>
        )}
        {approvals.length > 0 && (
          <span
            style={{
              marginLeft: "auto",
              fontSize: 12,
              color: COLOR,
              fontWeight: 500,
            }}
          >
            {approvals.length} approval{approvals.length !== 1 ? "s" : ""}
          </span>
        )}
      </div>

      {approvals.length === 0 ? (
        <div style={{ textAlign: "center", padding: "32px 0", color: "#999", fontSize: 14 }}>
          <span style={{ fontSize: 32, display: "block", marginBottom: 8 }}>📋</span>
          No Japan PMDA approval data found for this device.
        </div>
      ) : (
        <div style={{ padding: "20px 24px" }}>
          {/* Info panel */}
          <div
            style={{
              background: LIGHT_BG,
              border: `1px solid ${BORDER}`,
              borderRadius: 12,
              padding: "16px 20px",
              marginBottom: hasIndications ? 20 : 0,
              display: "flex",
              flexWrap: "wrap",
              gap: 24,
              alignItems: "center",
            }}
          >
            <div>
              <div style={labelStyle}>JP Status</div>
              <StatusBadge status="Approved" />
            </div>
            <div>
              <div style={labelStyle}>Regulator</div>
              <span style={{ fontSize: 13, color: "#333", fontWeight: 500 }}>
                PMDA (医薬品医療機器総合機構)
              </span>
            </div>
            <div>
              <div style={labelStyle}>Regulatory Framework</div>
              <span style={{ fontSize: 13, color: "#333", fontWeight: 500 }}>
                Pharmaceutical and Medical Device Act (PMD Act)
              </span>
            </div>
            {latestApproval?.approval_date && (
              <div>
                <div style={labelStyle}>Approval Date</div>
                <span style={{ fontSize: 13, color: "#333", fontWeight: 500 }}>
                  {new Date(latestApproval.approval_date).toLocaleDateString("en-CA")}
                </span>
              </div>
            )}
            <div style={{ marginLeft: "auto" }}>
              <a
                href="https://www.pmda.go.jp/english/review-services/reviews/approved-information/devices/0001.html"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  fontSize: 12,
                  color: COLOR,
                  textDecoration: "none",
                  fontWeight: 500,
                  display: "flex",
                  alignItems: "center",
                  gap: 4,
                }}
              >
                View on PMDA ↗
              </a>
            </div>
          </div>

          {/* Indications table */}
          {hasIndications && (
            <>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 12, fontWeight: 500 }}>
                Approved Indications (from PMDA classification, Grok-enriched)
              </div>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: LIGHT_BG }}>
                      <th style={thStyle}>Disease / Indication</th>
                      <th style={thStyle}>Status</th>
                      <th style={thStyle}>Approval Date</th>
                      <th style={thStyle}>PMDA Reference</th>
                    </tr>
                  </thead>
                  <tbody>
                    {approvals.filter((a) => a.disease_state).map((approval, idx) => (
                      <tr
                        key={approval.id}
                        style={{
                          borderTop: "1px solid rgba(0,0,0,0.04)",
                          background: idx % 2 === 0 ? "transparent" : "rgba(0,0,0,0.01)",
                        }}
                      >
                        <td style={tdStyle}>
                          <span style={{ fontWeight: 500, color: "#111" }}>
                            {approval.disease_state?.name ?? "—"}
                          </span>
                        </td>
                        <td style={tdStyle}>
                          <StatusBadge status={approval.status} />
                        </td>
                        <td style={tdStyle}>
                          <span style={{ fontSize: 12, color: "#555" }}>
                            {approval.approval_date
                              ? new Date(approval.approval_date).toLocaleDateString("en-CA")
                              : "—"}
                          </span>
                        </td>
                        <td style={tdStyle}>
                          {approval.source_ref ? (
                            <a
                              href={pmdaSearchUrl(approval.source_ref)}
                              target="_blank"
                              rel="noopener noreferrer"
                              style={{
                                fontFamily: "monospace",
                                fontSize: 11,
                                color: COLOR,
                                textDecoration: "none",
                              }}
                            >
                              {approval.source_ref.length > 40
                                ? approval.source_ref.slice(0, 40) + "…"
                                : approval.source_ref} ↗
                            </a>
                          ) : (
                            <span style={{ color: "#ccc" }}>—</span>
                          )}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            </>
          )}
        </div>
      )}
    </div>
  );
}

const labelStyle: React.CSSProperties = {
  fontSize: 10,
  color: "#888",
  fontWeight: 600,
  textTransform: "uppercase",
  letterSpacing: "0.06em",
  marginBottom: 4,
};

const thStyle: React.CSSProperties = {
  padding: "10px 16px",
  textAlign: "left",
  fontWeight: 600,
  color: "#555",
  fontSize: 11,
  letterSpacing: "0.06em",
  textTransform: "uppercase",
};

const tdStyle: React.CSSProperties = {
  padding: "12px 16px",
  verticalAlign: "top",
};
